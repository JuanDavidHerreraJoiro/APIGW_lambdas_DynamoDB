...
data "archive_file" "lambdas_students" {
  for_each = local.lambdas

  output_path = "files/${each.key}_lambdas_students.zip"
  type        = "zip"
  source_file = "${local.lambdas_path}/lambdas_students/${each.key}.js"
}

resource "aws_lambda_function" "lambdas_students" {
  for_each = local.lambdas
  ...
  filename         = data.archive_file.lambdas_students[each.key].output_path
  source_code_hash = data.archive_file.lambdas_students[each.key].output_base64sha256
...
resource "aws_lambda_permission" "api" {
  ...
  function_name = aws_lambda_function.lambdas_students[each.key].arn
